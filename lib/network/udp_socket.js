...
function initSocket(port, ip = '0.0.0.0') {
    ...
    socket.on('message', (msg, rinfo) => {
        ...
        if (parsedHeader.message_type === Protocol.MESSAGE_TYPES.ACK) {
            ...
            // Check if this is a Handshake packet and route it to the State Machine
            if (parsedHeader.message_type.includes('HANDSHAKE') || parsedHeader.message_type === Protocol.MESSAGE_TYPES.DISCONNECT) {

                // FUTURE: We need the peer role and a method to send the response here.
                // This structure will be finalized in the QA Integration test.

                // TEMPORARY LOGGING:
                Logger.log('NE', `Routing Handshake packet: ${parsedHeader.message_type} to PA State Machine.`);

                // Assume the main application has set the initial role via StateMachine.setPeerRole('HOST'/'JOINER')
                StateMachine.handleHandshakePacket(parsedHeader, StateMachine.peerRole(), sendResponse);
