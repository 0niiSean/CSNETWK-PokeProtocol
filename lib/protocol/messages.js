 ...
 * @param {number} ackNumber - The sequence_number of the request being acknowledged. // ADDED
 * @returns {Object} The complete message object ready for encoding.
 */
export function createHandshakeResponse(peerId, teamPreview, ackNumber) {
    if (!peerId || !teamPreview || teamPreview.length !== 3) {
        throw new Error("Invalid parameters for HANDSHAKE_RESPONSE creation.");
    }
    // Added validation for the new required ackNumber
    ...
    const message = {
        ...
        // FIX 1: Use RELIABILITY_FIELDS constant for sequence_number
        [RELIABILITY_FIELDS.SEQUENCE_NUMBER]: 0, 
        [HANDSHAKE_FIELDS.PEER_ID]: peerId,
        [HANDSHAKE_FIELDS.TEAM_PREVIEW]: teamPreview,
        [HANDSHAKE_FIELDS.TIMESTAMP]: Date.now(),
        // FIX 2: PIGGYBACK ACKNOWLEDGMENT
 ...
 */
export function createAttackAnnounceMessage(gameId, move, target) {
    return {
...
/**
 * Creates a CALCULATION_REPORT message object.
 */
export function createCalculationReportMessage(gameId, results) {
    return {
 ...
 */
export function createChatMessage(peerId, content, contentType = 'TEXT') {
    return {
        ...
        content: content,
        timestamp: Date.now(),
        // NOTE: Chat messages usually do NOT use the NE reliability layer's sequence_number,
        // but we include it for the NE team to handle if necessary.
    };
}

export function createGameOverMessage(gameId, winnerId) {
    return {
...
export function createResolutionRequestMessage(gameId, discrepancyData) {
    return {
...
export function createCalculationConfirmMessage(gameId, turn) {
    return {
...
export function createDefenseAnnounceMessage(gameId, defenseAction) {
    return {
